...
#### Solace PubSub+ for VMware Tanzu (formerly Pivotal Platform) Download
```
- name: solace-product-download
  type: docker-image
  source:
...
- name: solace-tile
  ...
  source:
    username: {{solace-product-username}}
    password: {{solace-product-password}}
    pivnet_token: {{uaa_refresh_token_for_pivnet}} # VMware Tanzu Network UAA token found in Settings
    accept_terms: true
...
- name: demo-resource
  ...
  [...]
```
This configuration will download the latest Solace PubSub+ Enterprise for VMware Tanzu. If `version` is specified, it will instead download the checksum of the specified version from Pivnet. This configuration requires a `pivnet_token` as your PivNet credentials in addition to the [Solace products](https://products.solace.com/) credentials.
With this configuration the `solace-tile` resource will download a checksum file from VMware Tanzu Network Solace Pubsub+ (https://network.pivotal.io/products/solace-pubsub).
The checksum file will be interpreted to download the corresponding product from [Solace products](https://products.solace.com/), the downloaded file will be verified using this checksum. Using the configuration implies user's acceptance of the Solace Systems Software License Agreement. You must also accept the EULA of Solace Pubsub+ on Vmware Tanzu found [here](https://network.pivotal.io/legal_documents/solace-eula) which is required to download from Tanzu Network.
...
* Solace Pubsub+ for Tanzu https://network.pivotal.io/products/solace-pubsub
