...
export SOLACE_PRODUCT_DOWNLOAD_TAG=${SOLACE_PRODUCT_DOWNLOAD_TAG:-"latest"}
...
docker build . -t "$SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME"

echo "Tagging $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME with $SOLACE_PRODUCT_DOWNLOAD_NAME:$SOLACE_PRODUCT_DOWNLOAD_TAG"
docker tag $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME:$SOLACE_PRODUCT_DOWNLOAD_TAG

if [ -z $DOCKER_LOCAL_ONLY ]; then
  if [ ! -z "$DOCKER_REGISTRY" ]; then
    echo "Using docker registry $DOCKER_REGISTRY"
    export SOLACE_PRODUCT_DOWNLOAD_DOCKER_TARGET="$DOCKER_REGISTRY/$SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME"
    echo "Tagging $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME with $SOLACE_PRODUCT_DOWNLOAD_DOCKER_TARGET"
    docker tag $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME $SOLACE_PRODUCT_DOWNLOAD_DOCKER_TARGET
  else
  ...
  if [ ! -z "$DOCKER_REGISTRY" ]; then
    echo "Pushing $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME:$SOLACE_PRODUCT_DOWNLOAD_TAG $DOCKER_REGISTRY"
    docker push "$DOCKER_REGISTRY/$SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME"
  else
    echo "Pushing $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME:$SOLACE_PRODUCT_DOWNLOAD_TAG $DOCKER_REGISTRY"
    docker push $SOLACE_PRODUCT_DOWNLOAD_DOCKER_NAME:$SOLACE_PRODUCT_DOWNLOAD_TAG
  fi
else
